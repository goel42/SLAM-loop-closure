...
def get_inverse_yelli_pose(pose_3d):
    #theta = -pose[2] + np.pi
    #theta = normalize_pose(theta)
    z = pose_3d[2]
    pose = np.array([pose_3d[0], pose_3d[1], pose_3d[-1]])
    R,t = get_inverse_yelli_transform(pose) 
    theta = get_angle_from_2D_rotation_mat(R) + np.pi/2 
    theta = normalize_pose(theta)
    return np.array([t[0], t[1], z, theta])


#WHAT IS?
def combine_yelli_poses(p1_3d, p2_3d):
    z1, z2 = p1_3d[2], p2_3d[2]
    if z1 != z2:
        print("z1-z2: ", z1,z2)
    assert z1==z2
    p1 = [p1_3d[0], p1_3d[1], p1_3d[-1]]
    p2 = [p2_3d[0], p2_3d[1], p2_3d[-1]]
    ...
    theta = get_angle_from_2D_rotation_mat(R) + np.pi/2
    theta = normalize_pose(theta)
    return np.array([t[0], t[1], z1, theta])
